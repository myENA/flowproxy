...
func worker(id int, ctx context.Context, server string, port int, wg *sync.WaitGroup, workerChan <-chan []byte) {
	...
	srcPort := common.RandomNum(10000, 15000)
	...
	for {
		...
		case payload := <-workerChan:
			...
			_, err = SendPacket(conn, &net.UDPAddr{IP: destIP, Port: port}, buf, false)
			if err != nil {
...
func statsPrinter(ctx context.Context, wg *sync.WaitGroup, rStats *RecordStat) {
	...
	for {
		...
		case <-time.After(time.Second * 10):
			...
			log.Printf("Netflow v9 Packets: %d IPFIX Packets: %d Other Packets: %d Total Pkts: %d pps: %d",
				rStats.Netflow9.Pkts, rStats.Ipfix.Pkts, rStats.OtherPkts, rStats.TotalPkts, pps)
		}
	}
}
...
// and discards others.
func parseNetflow(ctx context.Context, wg *sync.WaitGroup, proxyChan <-chan []byte, dataChan chan<- []byte, rStats *RecordStat, verbose bool) {
	defer wg.Done()
	// Start the loop
	for {
